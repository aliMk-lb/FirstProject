...
const FRONTEND_URL = (process.env.FRONTEND_URL || 'https://first-project-murex-sigma.vercel.app').replace(
  /\/$/,
  ''
)
...
const DB_USER_FALLBACK = process.env.DB_USER || process.env.MYSQLUSER || process.env.MYSQL_USER || 'root'
...
const DB_NAME_FALLBACK =
  process.env.DB_NAME || process.env.MYSQLDATABASE || process.env.MYSQL_DATABASE || 'online_consultation'
...
const allowedOrigins = new Set([...baseAllowedOrigins, ...additionalAllowedOrigins].filter(Boolean))

const isAllowedOrigin = (origin) => {
  if (!origin) return true // server-to-server / health / curl
...
app.get('/health', async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ ok: false, error: 'Database connection failed', detail: err?.message })
  }
})
